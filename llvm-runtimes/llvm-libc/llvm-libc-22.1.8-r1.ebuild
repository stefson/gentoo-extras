...
src_prepare() {
	...
	# Fix upstream issue #92337 where install-libc incorrectly requires 
	# libc-startup code during an overlay build.
	if [[ -f "${WORKDIR}/llvm-project-${PV}.src/libc/lib/CMakeLists.txt" ]]; then
		sed -i 's/"install-libc-headers;libc-startup"/"install-libc-headers"/g' \
